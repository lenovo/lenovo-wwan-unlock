...
SDX61_check=$(lspci -d :0308)
CURRENT_CONFIG=$(cat "$SERVICE_FILE")

restart_mm_service=false
function version_ge() { test "$(echo "$@" | tr " " "\n" | sort -rV | head -n 1)" == "$1"; }

if [ -n "$Rplus_check" ] || [ -n "$FM350_check" ] || [ -n "$RM520_check" ] || [ -n "$EM160R_check" ] || [ -n "$SDX61_check" ]; then
	curmmver=$(mmcli -V)
	first_line=${curmmver%%\n*}
	...
	echo "Issue Fix is only applicable for Fibocom L860-GL-16/FM350 ,Quectel EM160R-GL/RM520N-GL and Foxconn SDX61 WWAN module."
fi
